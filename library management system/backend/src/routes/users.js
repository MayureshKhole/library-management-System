...
router.post("/borrow", async (req, res, next) => {
  try {
    ...
    await book.update({
      borrowedBy: [...book.borrowedBy, user.id],
    })
...
router.post("/grant", async (req, res, next) => {
  try {
    ...
    if (book.borrowedBy.includes(req.body.userId)) {
      await book.update({
        grantedTo: [...book.grantedTo, req.body.userId],
      })
      const updatedBook = await BookModel.findById(book.id)
      return res.status(200).json({
        book: {
    ...
    return res.status(400).json({ error: "You've not borrowed this book" })
  } catch (err) {
    next(err)
  }
})
...
router.post("/return", async (req, res, next) => {
  try {
    ...
    await book.update({
      ...
      grantedTo: book.grantedTo.filter((grantedTo) => !grantedTo.equals(user.id)),
    })
    const updatedBook = await BookModel.findById(book.id)
    return res.status(200).json({
      book: {
...
router.get("/borrowed-books", async (req, res, next) => {
  try {
    ...
    const result = await BookModel.find({ borrowedBy: { $in: "6400a56595d66c9e8870d5dd" } })
    console.log("user borrow", req.session.userId)
